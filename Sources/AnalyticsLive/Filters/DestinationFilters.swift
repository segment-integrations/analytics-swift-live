...
public class DestinationFilters: UtilityPlugin {
    public var key: String = "DestinationFiltersPlugin"
    ...
    internal var settings: Settings? = nil
...
extension DestinationFilters: LivePluginsDependent {
    ...
    public func readyToStart() {
        ...
        rules?.forEach { rule in
            ...
            if !destination.isEmpty {
                let jsRule = rule.toJSConvertible()
                let createDestFilter = engine.value(for: "createDestinationFilter") as? JSFunction
                let added = createDestFilter?.call(args: [destination, jsRule])
    ...
    public func teardown(engine: JSEngine) {
        // remove any exisitng destination filters
        let removePrevFilters = engine.value(for: "removePreviousDestinationFilters") as? JSFunction
        removePrevFilters?.call(args: nil)
    }
}
